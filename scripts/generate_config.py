...
.env 파일의 Supabase 자격증명(SUPABASE_URL, SUPABASE_ANON_KEY)과 선택값(NEWS_API_KEY, API_BASE_URL)을
읽어 js/config.js를 재생성하는 스크립트.
...
- API_BASE_URL(선택값, Tab1 FastAPI 백엔드 주소)도 비어 있으면 로컬 개발 기본값
  (http://127.0.0.1:8000)으로 대체한다 -- js/api-client.js가 항상 이 값을 참조하므로
  NEWS_API_KEY와 달리 빈 문자열로 두지 않는다.
...
// .env에 API_BASE_URL이 비어 있으면 로컬 개발 기본값(uvicorn 기본 포트)으로 대체됩니다.
...
def main():
    ...
    api_base_url = env.get("API_BASE_URL", "").strip() or "http://127.0.0.1:8000"
    ...
    print("[INFO] API_BASE_URL: {}".format(api_base_url))
