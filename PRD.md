...
- **시작** (로그인 이전 랜딩): 서비스 소개 히어로, 핵심 기능 소개, 회원 유형(기업/구직자)별 안내, 이용 흐름, **"메인" 회원 전용 대시보드 미리보기 홍보 섹션**(2026-07-14 추가 — 실제 데이터는 노출하지 않고 트렌드/스킬랭킹/하이라이트/뉴스를 볼 수 있다는 것만 소개해 가입을 유도한다), 하단 가입 CTA. 로그인 여부와 무관하게 항상 접근 가능한 진입 화면이다.
- **메인** (로그인 회원 전용, 2026-07-14 `common` 브랜치 병합 반영): 통합 검색(히어로) → 역할 선택 CTA는 생략(이미 가입한 회원 전용이라 불필요) → 채용 트렌드(우리 플랫폼 job_postings 기준) → 실시간 채용 시장 동향(원티드 API 기준) → 스킬 수요 랭킹 & 조합 분석 → 추천 하이라이트(구직자는 매칭 공고, 기업은 매칭 인재) → 최근 공고/최근 인재 → 채용 뉴스 순으로 구성된 종합 대시보드다. **로그인하지 않은 상태로 "메인" 탭에 진입하면 대시보드 대신 가입 유도 게이트만 보인다** — 회원가입 유인 전략(2026-07-14 결정)에 따라 의도적으로 로그인 회원에게만 공개한다.
...
> "채용 뉴스"는 "메인" 탭에 구현되어 있다(`js/news.js`, 무료 뉴스 API 연동 + 정적 폴백, `common` 브랜치 병합분). 기업·구직자 어느 쪽에도 치우치지 않는 "메인" 화면에만 노출하고 Tab1/Tab2에는 노출하지 않는다.
