...
| 변수명 | HEX | 용도 | 사용 강도 |
|---|---|---|---|
| `--pink-primary` | `#FF6B9D` | 핵심 브랜드 컬러. Primary 버튼 배경, 링크 언더라인, 매칭 스코어 링, 히어로 배경 그라데이션 | 매우 높음 (전면 사용) |
...
**섹션 그룹핑 (2026-07-14 개정)**: 메인은 서로 다른 성격의 섹션 8개(히어로/역할CTA/자체트렌드/원티드트렌드/스킬랭킹·조합/추천하이라이트/최근공고·인재/뉴스)를 한 페이지에 순서대로 배치하는데, 정보량은 줄이지 않되 "성격이 같은 섹션끼리는 시각적으로 한 덩어리"라는 단서를 주기 위해 5개 그룹으로 묶는다. 그룹 순서와 내부 섹션 순서는 그대로 유지한다.

| 그룹 | 구성 섹션 | 시각 처리 |
|---|---|---|
| A. 진입 | 히어로 검색 + 역할 선택 CTA | 히어로는 `--pink-tint-50` wash로 이미 구분됨(기존 유지) |
| B. 시장 데이터 | 채용 트렌드(자체) + 실시간 채용 시장 동향(원티드) + 스킬 랭킹/조합 | `.section-group` 컨테이너(배경 `--paper-dim` wash + `--line` 보더 + 오버라인 라벨 "시장 데이터")로 3개 섹션을 한 덩어리로 묶음. 각 섹션의 h2/h3와 출처 라벨("우리 플랫폼 기준" / "원티드 제공")은 그대로 유지 — 그룹핑은 시각적 묶음일 뿐 데이터 출처 구분을 지우지 않는다. 채용 트렌드(자체)의 ▲/▼ 증감률은 `job_postings.posted_at` 기준 최근 14일 vs 그 이전 14일 건수를 비교한 실측값이다(가공/추정 수치 아님) — 원티드 트렌드는 스냅샷에 개별 게시일이 없어 증감률을 붙이지 않는다 |
| C. 개인화 | 추천 하이라이트 캐러셀 | 기존 강조 톤 유지(가로 스크롤 캐러셀) |
| D. 최근 등록 | 최근 공고 + 최근 인재 (2열) | 좌우 폭·비중 대칭 유지 |
| E. 읽을거리 | 채용 뉴스 | 최하단, 통계 그룹과 다른 톤(카드 리스트) |

그룹 B에만 별도 배경 컨테이너(`.section-group`)를 쓰는 이유는 이 3개 섹션만 "통계 묶음"이라는 유일한 공통 성격을 가지기 때문이고, A/C/D/E는 원래도 각 1개 섹션(A는 히어로+CTA 2개지만 이미 히어로 배경으로 구분됨)이라 별도 컨테이너 없이 기존 `.main-section` 간격 리듬만으로 충분하다. 오버라인 라벨(12px uppercase, `--text-tag` 토큰 재사용, `--pink-active` 텍스트)은 "그룹 > 섹션(h2) > 본문" 3단 위계를 시각적으로 알리는 표식이며, 시각 전용이라 `aria-hidden`, 그룹의 접근성 이름은 `<section aria-label="시장 데이터 섹션 그룹">`으로 별도 제공한다(5.2절/7.2절 접근성 원칙과 일관).

간격은 "그룹 간 간격 > 그룹 내부 섹션 간 간격"으로 항상 차등화한다(4.4절 스페이싱 토큰만 사용, 새 값 도입 없음) — `.main-section`/`.section-group`(그룹 레벨)은 브레이크포인트별로 모바일 `--space-8`(32px) → 태블릿 `--space-12`(48px) → 데스크톱 `--space-16`(64px)로 확대되고, 그룹 B 내부 섹션 간격(`.section-group__item`)은 브레이크포인트와 무관하게 `--space-6`(24px) 고정이라 어떤 화면 크기에서도 항상 그룹 간 간격보다 좁다.

2026-07-14 개정: 탭 네비게이션 인디케이터를 언더라인에서 pill(선택 시 `--pink-tint-50` 배경 + `--pink-active` 텍스트, 비선택은 투명)로 교체했다(New.html 반영). 스킬 수요 랭킹도 태그 뱃지 대신 채용 트렌드와 동일한 바(rank-bar) 형태로 통일했다.

```
+--------------------------------------------------------+
| [로고]     메인 | 기업(Tab1) | 구직자(Tab2)     [로그인] |
+--------------------------------------------------------+
|  Group A          HERO (pink-tint-50 배경)               |
|        "카테고리 기반으로, 꼭 맞는 채용을 찾다"           |
|   [직무▾][지역▾][ 스킬/회사명 자유 검색 ............ ][검색]| (2026-07-14: New.html 반영, pill 컨테이너 + 직무/지역 select 2개 추가)
|        [기업으로 시작하기]   [구직자로 시작하기]         |
+--------------------------------------------------------+
|  Group B  ── 시장 데이터 ── (paper-dim wash 컨테이너)     |
|   채용 트렌드(우리 플랫폼 기준)      [바 랭킹 + ▲▼증감률]  |
|   실시간 채용 시장 동향(원티드 제공)  [바 랭킹]            |
|   스킬 수요 랭킹 & 조합 분석          [바 랭킹][콤보 리스트]|
+--------------------------------------------------------+
|  Group C  추천 하이라이트 (매칭 스코어 카드, 가로 스크롤) →|
|  [카드][카드][카드][카드][카드] ...                      |
+--------------------------------------------------------+
|  Group D  최근 공고             |  최근 인재(로그인 유형별 분기) |
|  [공고카드][공고카드]           |  [인재카드][인재카드]         |
+--------------------------------------------------------+
|  Group E  채용 뉴스 (카드 리스트)                         |
+--------------------------------------------------------+
```
...
> 메인 페이지의 "그룹 간 간격 > 그룹 내부 섹션 간 간격" 차등 적용 규칙과 실제 클래스(`.main-section`/`.section-group`/`.section-group__item`)는 4.1절 참고. 새 spacing 토큰을 추가하지 않고 아래 스케일만 재사용한다.
...
**그림자 원칙 (2026-07-14 정리)**: 정적(기본) 상태에는 그림자를 쓰지 않고, `hover`/`focus-within`에만 그림자를 부여한다 — 카드가 인터랙션 가능한 요소임을 그림자 자체로 신호하기 위함이다. 이 원칙은 `.card`뿐 아니라 카드와 시각적으로 유사한 정적 정보 블록(예: 4.1절 그룹 B의 랭킹 바 `.rank-bar`, 스킬 조합 리스트 `.combo-list__item`)에도 일관 적용한다 — 다만 이들은 클릭/hover 인터랙션이 없는 순수 정보 표시 요소이므로 hover 그림자도 추가하지 않고 그림자 자체를 아예 쓰지 않는다(그림자 유무로 "이건 누를 수 있다/없다"를 구분).
...
.card:focus-within {
  ...
  box-shadow: 0 12px 24px rgba(255, 107, 157, 0.18); /* pink-primary 18% — 그림자는 hover/focus 상태에만 존재 */
}
