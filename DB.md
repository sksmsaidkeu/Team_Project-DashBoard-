...
>
> 2026-07-13: `supabase/migrations/`에 이 문서를 그대로 반영한 SQL 마이그레이션(스키마 + RLS)을 작성했습니다.
> 다만 실제 Supabase 프로젝트는 팀원 소유라 이 세션에서 직접 적용하지는 못했으며, 담당 팀원이
> Supabase SQL Editor(또는 `supabase db push`)로 적용해야 합니다.
...
- Row Level Security(RLS) 정책은 `supabase/migrations/20260713130000_rls_policies.sql`에 정의되어 있다(2026-07-13 확정). 원칙: `categories`/`company_profiles`/`company_profile_skills`/`job_postings`/`job_posting_position_details`는 시장 정보라 공개 SELECT, 쓰기는 소유자(또는 `categories`는 service_role)만. `users`/`user_preferences`/`interaction_logs`는 본인만. `jobseeker_profiles`/`jobseeker_profile_skills`는 `is_region_public`과 `is_salary_public`이 **모두 true**인 행만 타인에게 전체 공개하는 단순 정책을 채택했다(컬럼 단위 마스킹 아님 — 하나라도 false면 본인만 조회 가능. 더 세밀한 공개가 필요해지면 뷰 기반으로 재설계 필요). `wanted_job_trend_snapshot`(3.10절, `supabase/migrations/20260714000000_wanted_job_trend_snapshot.sql`)도 시장 정보이므로 공개 SELECT, 쓰기는 `categories`와 동일하게 정책 없음(service_role 전용).
...
- **API 시드 가능 범위 재검증 (실제 호출로 확인, `scripts/seed_categories.py`)**: `JOB`은 `GET /tags/categories`로 전체 시드가 가능하다(직군 20건/직무 418건, 파라미터 불필요). 단 응답 필드명은 `tag_id`가 아니라 `id`이며, `categories.tag_id` 컬럼명은 이 값을 저장하는 우리 쪽 명명일 뿐 원티드 응답 필드명과 동일한 것은 아니다. `SKILL`(`/tags/skills`)은 `keyword` 필수 파라미터라 전체 목록 조회가 불가하고, `INDUSTRY`(`/insight/company`)는 `biz_number` 필수 파라미터라 전체 KSIC 목록 조회 용도가 아니다 — 이 둘은 원티드 API만으로 전체 시드가 불가능하므로, `SKILL`은 키워드 반복 호출/수동 큐레이션으로, `INDUSTRY`는 KSIC 공개 데이터 정적 시드로 대체한다(PRD 3장 참고).
...
### 3.10 `wanted_job_trend_snapshot`

**실시간 원티드 채용 동향** 스냅샷. `job_postings` 기반의 기존 "채용 트렌드"(7.1절, 아직 자체 등록 공고가 적어 데이터가 희소)와는 별개로, 원티드(Wanted) 실제 라이브 공고를 원티드 API에서 직접 집계한 지표다. `scripts/fetch_wanted_trend.py`가 `GET /jobs`(v1 — openapi.json엔 "v2 api 사용 권장"으로 deprecated 표시돼 있으나, 이 저장소엔 v2 스펙이 없어 이번엔 v1을 그대로 사용)를 페이지네이션 호출해 `status='active'` 공고를 `category_tags.parent_tag`(직군)별로 집계한 뒤 upsert한다.

| 컬럼명 | 타입 | 제약조건 | 설명 |
|---|---|---|---|
| `id` | `uuid` | PK, DEFAULT `gen_random_uuid()` | |
| `tag_id` | `integer` | NOT NULL | 원티드 `category_tags.parent_tag.id`(직군 태그 ID) |
| `title` | `text` | NOT NULL | 원티드 `category_tags.parent_tag.title` |
| `job_count` | `integer` | NOT NULL, CHECK (`job_count` >= 0) | 해당 스냅샷 시점에 집계된 active 공고 수 |
| `snapshot_at` | `timestamptz` | NOT NULL | 스크립트 실행(집계) 시각 |
| `created_at` | `timestamptz` | NOT NULL, DEFAULT `now()` | |

- `UNIQUE (tag_id, snapshot_at)` — 재실행 시 같은 스냅샷 회차 내 중복 upsert 방지, 스냅샷 이력은 계속 누적 보존.
- 프론트는 `WHERE snapshot_at = (SELECT MAX(snapshot_at) FROM wanted_job_trend_snapshot)`로 최신 스냅샷 랭킹만 조회하면 된다(`scripts/fetch_wanted_trend.py` 주석 참고).
...
| 민감정보(연봉/지역) 비공개 옵션 | `jobseeker_profiles`에 `is_salary_public`, `is_region_public` 불리언 컬럼을 추가하고 기본값은 `true`(공개)로 설정했다. RLS 정책 확정(2026-07-13, 더 이상 미확정 이슈 아님): 두 값이 모두 `true`인 행만 타인에게 공개, 하나라도 `false`면 본인만 조회 가능(컬럼 단위 마스킹은 채택하지 않음). |
